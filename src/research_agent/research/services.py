...
async def multi_omics_fusion(payload: dict[str, Any]) -> CapabilityResult:
    ...
    Uses the unified data layer (:mod:`unified_data`) for parsing, validation,
    ...
    try:
        scrna_unified, scrna_warnings = read_matrix_from_store(
            store, scrna_artifact, user_id, run_id, "scRNA-seq"
        )
        spatial_unified, spatial_warnings = read_matrix_from_store(
            store, spatial_artifact, user_id, run_id, "spatial"
        )
    except ValueError as exc:
        return CapabilityResult(
            status="failed",
    ...
    all_warnings = list(scrna_warnings) + list(spatial_warnings)

    scrna_gene_to_idx = {g: i for i, g in enumerate(scrna_unified.features)}
    spatial_gene_to_idx = {g: i for i, g in enumerate(spatial_unified.features)}
    common_genes = [g for g in scrna_unified.features if g in spatial_gene_to_idx]
    if not common_genes:
        return CapabilityResult(
            status="failed",
            output={
                "message": "scRNA-seq 与空间数据之间没有共同的基因标识符，无法融合。",
                "scrna_genes_found": len(scrna_unified.features),
                "spatial_genes_found": len(spatial_unified.features),
            },
    ...
    n_scrna_cells = len(scrna_unified.samples)
    n_spatial_spots = len(spatial_unified.samples)

    # Normalise each matrix column-wise (per-sample) via the shared utility.
    scrna_norm = log1p_zscore(scrna_unified.values, n_scrna_cells)
    spatial_norm = log1p_zscore(spatial_unified.values, n_spatial_spots)
    ...
    # Quality metrics (computed on raw values before normalisation).
    scrna_total_counts = [
        sum(max(scrna_unified.values[r][c], 0.0) for r in range(len(scrna_unified.features)))
        for c in range(n_scrna_cells)
    ]
    spatial_total_counts = [
        sum(max(spatial_unified.values[r][c], 0.0) for r in range(len(spatial_unified.features)))
        for c in range(n_spatial_spots)
    ]
    ...
    warnings_list = list(all_warnings)
    if scrna_median < 100:
    ...
    if len(common_genes) < min(len(scrna_unified.features), len(spatial_unified.features)) * 0.3:
        warnings_list.append(
            f"共同基因比例低于30%（{len(common_genes)}/{min(len(scrna_unified.features), len(spatial_unified.features))}），融合结果可能不完整。"
        )

    header_cols = (
        [f"cell_c{j}" for j in range(n_scrna_cells)]
    ...
    generated_artifacts = [
        {
            ...
            "type": "table",
            "shape": {"genes": len(common_genes), "cells": n_scrna_cells, "spots": n_spatial_spots},
        }
    ]
    ...
    output = {
        ...
        "scrna_genes": len(scrna_unified.features),
        "spatial_genes": len(spatial_unified.features),
        "scrna_cells": n_scrna_cells,
        "spatial_spots": n_spatial_spots,
        "scrna_median_umis": round(scrna_median, 2),
        "spatial_median_counts": round(spatial_median, 2),
        "fused_matrix_shape": [len(common_genes), n_scrna_cells + n_spatial_spots],
        "joint_analysis": {
            "method": "log1p-zscore alignment on common gene space (unified_data layer)",
            "recommendations": [
        ...
        "message": f"成功融合 {len(common_genes)} 个共同基因，{n_scrna_cells} 个细胞 x {n_spatial_spots} 个spot。",
    }
